...
class HouseFeatures(BaseModel):
    ...
    TotalBsmtSF: float = Field(
        ..., ge=0, description="Total square feet of basement area"
    )
    ...
    YearBuilt: int = Field(
        ..., ge=1800, le=2026, description="Original construction date"
    )
...
def start():
    ...
    uvicorn.run(
        "house_price_prediction.webapp.main:app", host="0.0.0.0", port=8000, reload=True
    )
